...
`Hummingbird <http://birdhouse-hummingbird.readthedocs.org/en/latest/>`_ provides a `Web Processing Service (WPS) <http://www.opengeospatial.org/standards/wps>`_ using `PyWPS <http://pywps.org>`_ for the climate science community. It has WPS processes for common tools used in climate science like `CDO <https://code.zmaw.de/projects/cdo>`_ and `cfchecker <https://pypi.python.org/pypi/cfchecker>`_. 
